...
<template>
  <div class="sales-view">
    ...
    <section class="sales-card">
      ...
      <div v-else class="sales-table-wrap"><table><thead><tr><th>Customer</th><th>Reference</th><th>Publication</th><th>Amount</th><th>Status</th><th>Email</th><th>Downloads</th><th>Date</th></tr></thead><tbody><tr v-for="order in orders" :key="order.id"><td><strong>{{ order.customerName || 'Customer' }}</strong><small>{{ order.customerEmail }}</small></td><td>{{ order.reference }}</td><td>{{ order.book.title }}</td><td>{{ money(order.amountMinor, order.currency) }}</td><td><span class="status-pill" :class="order.status.toLowerCase()">{{ order.status }}</span></td><td><span v-if="order.receiptEmailSentAt" class="email-state sent">Sent</span><span v-else-if="order.receiptEmailError" class="email-state failed" :title="order.receiptEmailError">Failed</span><span v-else class="email-state">Not sent</span></td><td>{{ order.downloadGrant ? `${order.downloadGrant.downloadCount}/${order.downloadGrant.maxDownloads}` : '—' }}</td><td>{{ date(order.createdAt) }}</td></tr><tr v-if="!orders.length"><td colspan="8" class="empty-cell">No orders match this filter.</td></tr></tbody></table></div>
